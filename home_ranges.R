...
PERCENT_SAMPLE_HOME_RANGES=95 # winsorizing parameter to cap the home ranges computations
...
waypoints_for_group <- function(
  ...
  # Calculate the home range size (area) for the group using the helper function
  home_range_info <- home_range_size_for_group(waypoints, group_label, percent)
  ...
  annotation_text <- sprintf(
    "<div style='background-color: rgba(0, 0, 0, 0.5); color: white; padding: 10px; border-radius: 5px;'>
      <b>Monkey Species:</b> %s<br>
      <b>Sightings in Convex Hull:</b> %d<br>
      <b>Percentage Used:</b> %.1f%%<br>
      <b>Home Range Size:</b> %.3f ha
    </div>",
    group_label,
    nrow(filtered_waypoints),
    percent,
    home_range_info$Area
  )
...
# (0)
#
overall_map=map_waypoints_all_groups(waypoints, TILE_PROVIDER)
#
# (1)
#
Wangwamani_map=waypoints_for_group(waypoints, group_label="Wangwamani", tile_provider = TILE_PROVIDER)
deep_forest_map=waypoints_for_group(waypoints, group_label="Deep Forest", tile_provider = TILE_PROVIDER)
joshi_map=waypoints_for_group(waypoints, group_label="Joshi", tile_provider = TILE_PROVIDER)
camp_map=waypoints_for_group(waypoints, group_label="Camp", tile_provider = TILE_PROVIDER)
#
# (2) Overall Home Ranges
#
home_range_sizes <- waypoints$species_group %>%
  unique() %>%
  lapply(function(each_group) {
    # Loop through the desired percent values for each group
    lapply(c(95, 97.5, 99), function(each_percent) {
      home_range_size_for_group(
        waypoints,
        group_label = each_group,
        percent = each_percent
      )
    }) %>%
      bind_rows() # Combine results for all percent values within a group
  }) %>%
  bind_rows() %>% # Combine results for all groups
  mutate(Methodology = "Annual-Overall") %>%
  arrange(Group, Percent) # Sort by Group and Percent
#
home_range_sizes %>% View()
#
# (3) Monthly Home Ranges
#
monthly_home_range_sizes <- waypoints %>%
  ...
  bind_rows() %>%
  mutate(Methodology = "Monthly")
#
monthly_home_range_sizes %>% View()
#
# (4)
#
monthly_home_ranges_plot=plot_monthly_home_range(monthly_home_range_sizes, percent_threshold = PERCENT_SAMPLE_HOME_RANGES)
#
# (5)
#
comparison_between_groups=
  monthly_home_range_sizes %>% 
  dplyr::mutate(
    parameter="Home Range Area (HA)"
  ) %>%
...
#
comparison_between_groups %>% View()

#
# 6. Export a module level report
#
...
report_items=list(
              ...
              group_maps=list(Wangwamani_map, deep_forest_map, joshi_map, camp_map ),
              
              home_ranges_sizes=list(home_range_sizes, monthly_home_range_sizes),
              
              #monthly_home_ranges_plot=monthly_home_ranges_plot,
              
              comparison_between_groups=list(comparison_between_groups, monthly_home_ranges_plot)
              )
#
report_items %>%lapply(class)
#
# (1)
#
setwd(HOME_RANGES_HOME)
report_items %>% excel_report()
