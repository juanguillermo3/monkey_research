...
# (4)
#
monthly_home_ranges_plot=plot_monthly_home_range(monthly_home_range_sizes, percent_threshold = 98)
...
excel_report <- function(report_items, file_name = "report.xlsx", width = 10, height = 6) {
  ...
  process_item <- function(item, sheet_name = "Sheet 1") {
    if (inherits(item, "data.frame")) {
      ...
      # If the type is not handled, print a warning
      warning(paste("Unsupported item type:", class(item), "for sheet:", sheet_name))
    }
  }
